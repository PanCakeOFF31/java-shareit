...
public class BookingIntegrationTest {
    ...
    class OrderedIntegrationTestWithoutRollback {
        ...
        public void test_T1040_PS01_getAllBookingByBooker() {
            List<BookingResponseDto> bookingsAll = bookingController.getAllBookingByBooker(booker2Id, "ALL", from, size);
            assertNotNull(bookingsAll);
            assertEquals(3, bookingsAll.size());

            List<BookingResponseDto> bookingsWaiting = bookingController.getAllBookingByBooker(booker2Id, "WAITING", from, size);
            assertNotNull(bookingsWaiting);
            assertEquals(2, bookingsWaiting.size());

            List<BookingResponseDto> bookingsRejected = bookingController.getAllBookingByBooker(booker2Id, "REJECTED", from, size);
        ...
        public void test_T1050_PS01_getAllBookingByOwner() {
            List<BookingResponseDto> bookingsAll = bookingController.getAllBookingByOwner(owner3Id, "ALL", from, size);
            assertNotNull(bookingsAll);
            assertEquals(3, bookingsAll.size());

            List<BookingResponseDto> bookingsApproved = bookingController.getAllBookingByOwner(owner3Id, "FUTURE", from, size);
            assertNotNull(bookingsApproved);
            assertEquals(2, bookingsApproved.size());

            List<BookingResponseDto> bookingsRejected = bookingController.getAllBookingByOwner(owner3Id, "REJECTED", from, size);
            assertNotNull(bookingsRejected);
            assertEquals(1, bookingsRejected.size());
        }
