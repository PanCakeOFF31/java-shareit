...
class ItemResponseDtoJsonTest {
    ...
    public void test_T0010_PS01_foolField() throws IOException {
        ...
        BookingOrderResponseDto nextBooking = new BookingOrderResponseDto(nextBookingId, bookerId, ldt, ldt.plusHours(1));
        BookingOrderResponseDto lastBooking = new BookingOrderResponseDto(lastBookingId, bookerId, ldt.plusHours(2), ldt.plusHours(4));
        CommentResponseDto comment1 = new CommentResponseDto(comment1Id, "some-text", "maxim", ldt.plusDays(1));
        CommentResponseDto comment2 = new CommentResponseDto(comment2Id, "some-text", "martin", ldt.plusDays(2));

        List<CommentResponseDto> comments = List.of(comment1, comment2);

        ItemResponseDto itemResponseDto =
                new ItemResponseDto(itemId, "item-name", "some-description", false, lastBooking, nextBooking, comments, requestId);
    ...
    public void test_T0010_NS01_emptyField() throws IOException {
        ItemResponseDto itemBooking = new ItemResponseDto();
